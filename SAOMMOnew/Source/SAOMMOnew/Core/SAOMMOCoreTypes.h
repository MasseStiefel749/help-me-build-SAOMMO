...
#include "SAOMMOCoreTypes.generated.h"
 ...
 */
UENUM(BlueprintType)
enum class ESAOInputDevice : uint8
{
 ...
 */
UENUM(BlueprintType)
enum class ESAOCameraMode : uint8
{
...
struct FSAOInputFrame
{
	GENERATED_BODY()

	/** Planar locomotion intent, already in character space (X = forward, Y = right). */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "SAO Input")
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "SAO Input")
	ESAOInputDevice SourceDevice = ESAOInputDevice::Unknown;
	...
	void Reset()
	{
		...
		bAttack = false;
		bMenu = false;
		SourceDevice = ESAOInputDevice::Unknown;
	}
};
